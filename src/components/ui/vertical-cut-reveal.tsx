...
const VerticalCutReveal = forwardRef<VerticalCutRevealRef, TextProps>(({
  ...
  return (
    <span className={`flex flex-wrap whitespace-pre-wrap ${containerClassName ?? ''} ${splitBy === "lines" ? "flex-col" : ""}`} onClick={onClick} ref={containerRef} {...props}>
        ...
        return (
          <span key={wordIndex} aria-hidden="true" className={`inline-flex overflow-hidden ${wordLevelClassName ?? ''}`}>
            {wordObj.characters.map((char, charIndex) => (
              <span className={`whitespace-pre-wrap relative ${elementLevelClassName ?? ''}`} key={charIndex}>
