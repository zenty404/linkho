              ...
              >
                <div
                  className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${
                    isActive ? 'bg-white/20 text-white' : isSupport ? 'bg-brand/20 text-brand' : 'bg-navy/10 text-navy'
                  }`}
                >
                  {isSupport ? <Headphones size={15} /> : getInitials(conv.interlocuteurNom)}
                </div>
      ...
      {convParam === null ? (
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2">
            {messages.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-8">Démarrez la conversation</p>
            )}
            {messages.map((msg) => (
